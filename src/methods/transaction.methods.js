...
const getAllTransactions = async (offsetParam, limitParam) => {
  ...
  let data = await fetchFromBankApi(`${url}?${startDateParam}&${endDateParam}&limit=${limit}&offset=${offset}&order=desc`);
...
export const addNewTransaction = (data) =>
  ...
  data.forEach(async(transaction) => {
    ...
    if(new Date(getCardDetails?.user?.createdAt) < new Date(transaction.date)){
      const parsedTransactionDetails=JSON.parse(transaction.details);
      const relatedTransactions=parsedTransactionDetails?.relatedTransactions;
      let relatedAmount=0;
      relatedTransactions?.forEach((relatedTransaction)=>{
        relatedAmount+=relatedTransaction?.amount
      });
      queryQueue.add({ cardId: transaction.cardId, amount: transaction.amount+relatedAmount });
    }
  });
