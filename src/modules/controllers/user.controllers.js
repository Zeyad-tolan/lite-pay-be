...
import { filterByDateRange } from "../../methods/user.methods.js";
...
export const autoRequestsListenerForVipRole = ErrorHandlerService(async (req, res) => {
  ...
  if (totalUsdAmount >= 1000) {
    const findVipRole = await roleModel.findOne({
      where: { type: "vip" }
    });

    if (!findVipRole) return res.status(404).json({ message: "VIP role not found" });

    findUser.roleId = findVipRole.id;
    await findUser.save();

    return res.status(200).json({
      message: `User role changed to ${findVipRole?.type}`
    });
  }
...
export const ApplyMyRequestsAndTransactionsFilters = ErrorHandlerService(async (req, res) => {
  const { pageNo = 1, limit = 10, sort = 'asc', status,whatDate } = req.query;
  ...
  const sortedData = filteredData.sort((a, b) => {
    if (sort.toLowerCase() === 'asc') {
      if(a.transactionId && !b.transactionId){
        return new Date(a.bankCreatedAt) - new Date(b.createdAt);
      }
      else if(!a.transactionId && b.transactionId){
        return new Date(a.createdAt) - new Date(b.bankCreatedAt);
      }
      else if(a.transactionId && b.transactionId){
        return new Date(a.bankCreatedAt) - new Date(b.bankCreatedAt);
      }
      else{
      ...
      if(a.transactionId && !b.transactionId){
        return new Date(b.bankCreatedAt) - new Date(a.createdAt);
      }
      else if(!a.transactionId && b.transactionId){
        return new Date(b.createdAt) - new Date(a.bankCreatedAt);
      }
      else if(a.transactionId && b.transactionId){
        return new Date(b.bankCreatedAt) - new Date(a.bankCreatedAt);
      }
      else{
